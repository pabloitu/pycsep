...
# PyCSEP imports
from csep.utils.constants import SECONDS_PER_DAY, SECONDS_PER_HOUR, CSEP_MW_BINS
...
def plot_cumulative_events_versus_time(catalog_forecast,
                                       observation,
                                       bins=100,
                                       figsize=None,
                                       ...
                                       **kwargs):
    """
    Plot the cumulative number of forecasted events versus observations.
    ...
    if catalog_forecast.start_time is None or catalog_forecast.end_time is None:
        ...
        for ses in catalog_forecast:
            if ses.start_time and ses.end_time:
                extreme_times.append((datetime_to_utc_epoch(ses.start_time),
                                      datetime_to_utc_epoch(ses.end_time)))
        start_time = numpy.min(numpy.array(extreme_times))
        end_time = numpy.max(numpy.array(extreme_times))
    else:
        start_time = catalog_forecast.start_time
        end_time = catalog_forecast.end_time

    # offsets to start at 0 time and converts from millis to hours
    time_bins, dt = numpy.linspace(start_time,
                                   end_time,
                                   bins,
    ...
    millis_to_days = SECONDS_PER_DAY * 1000
    time_bins = (time_bins - time_bins[0]) / millis_to_days
    time_bins = time_bins + (dt / millis_to_days)
...
def plot_magnitude_vs_time(catalog: "CSEPCatalog",
                           figsize=None,
                           xlabel='Datetime',
                           ylabel='$M$',
    ...
    ax.scatter(datetimes, mag,
               marker="o", c=color,
               cmap=cmap,
               s=autosize_scatter(size, mag, mag_scale),
               alpha=0.3)
    ...
    # Autoformat the xticks and xticklabels
...
def autoscale_histogram(ax, bin_edges, simulated, observation):
    upper_xlim = numpy.percentile(simulated, 99.75)
    upper_xlim = numpy.max([upper_xlim, numpy.max(observation)])
    d_bin = bin_edges[1] - bin_edges[0]
    upper_xlim = upper_xlim + 2 * d_bin
...
def plot_dist_test(
    ...
    Args:
        ...
        observation (numpy.array or scalar): Observation to plot against stochastic event set.
        bins (str or int): Binning strategy (default: 'fd').
        percentile (float): Percentile for shading regions (default: None).
        show (bool): Show the plot (default: False).
        axes (matplotlib.Axes): Axes object to plot on (default: None).
        catalog (object): Catalog used for annotating the figures (default: None).
        figsize (tuple): Figure size (default: None).
        sim_label (str): Label for the simulated data (default: 'Simulated').
        obs_label (str): Label for the observation data (default: 'Observation').
        xlabel (str): Label for the X-axis (default: 'X').
        ylabel (str): Label for the Y-axis (default: 'Frequency').
        xycoords (tuple): Coordinates for annotations (default: (1.00, 0.40)).
        ...
        color (str): Color of the simulated data histogram (default: None).
        filename (str): Filename to save the figure (default: None).
        xlim (tuple): X-axis limits (default: None).
        annotate (bool): Whether to annotate the plot with text (default: False).
        annotation_text (str): Text to annotate the plot with (default: None).
        annotation_xy (tuple): Coordinates for the annotation (default: (0.55, 0.3)).
        annotation_fontsize (int): Font size for the annotation (default: 14).
        tight_layout (bool): Use tight layout (default: True).

    Returns:
        matplotlib.Axes: Matplotlib axes handle.
    """

    simulated = evaluation_result.test_distribution
    observation = evaluation_result.observed_statistic

    xlabel, ylabel, annotation_text, annotation_xy = get_plot_details(evaluation_result)
    # Initialize axes
    if axes is not None:
        ax = axes
    else:
    ...
    n, bin_edges, patches = ax.hist(simulated, bins=bins, label=sim_label, color='steelblue',
                                    alpha=alpha, edgecolor=None, linewidth=0)
    if isinstance(observation, (float, int)):
        ax.axvline(x=observation, color='black', linestyle='--',
                   label=obs_label + numpy.isnan(observation) * ' (NaN)')
    else:
        observation = observation[~numpy.isnan(observation)]
        ax.hist(observation, bins=bins, label=obs_label, edgecolor=None,
                linewidth=0, alpha=alpha, color='green')
    ...
    # Format axis object
    if xlim is None:
        ax = autoscale_histogram(ax, bin_edges, simulated, observation)
    else:
        ax.set_xlim(xlim)
    ax.set_title(title)
    ax.set_xlabel(xlabel)
    ...
    if annotate and annotation_text:
        ax.annotate(annotation_text, xycoords='axes fraction', xy=annotation_xy,
...
def plot_distribution_test(evaluation_result, axes=None, show=True,
                           plot_args=None):
    """
    Takes result from evaluation and generates a specific histogram plot to show the results of
     the statistical evaluation for the M-test.
...
def plot_catalog(catalog, ax=None, show=False, extent=None, set_global=False,
    ...
    if legend:
        ...
        handles, labels = scatter.legend_elements(prop="sizes",
                                                  num=list(autosize_scatter(markersize,
                                                                            mag_ticks,
                                                                            mag_scale)),
                                                  alpha=0.3)
...
def autosize_scatter(markersize, values, scale):
    if isinstance(scale, (int, float)):
        ...
        return (markersize / (scale ** min(values)) * numpy.power(values,
                                                                  scale))
...
def get_plot_details(evaluation_result):
    ...
    try:
        if evaluation_result.name == 'Catalog N-Test':
            ...
            try:
                ...
                # if both quantiles are provided, we want to plot the greater-equal quantile
                annotation_text = (
                    f'$\\gamma = P(X \\leq x) = {evaluation_result.quantile[1]:.2f}$\n'
                    f'$\\omega = {evaluation_result.observed_statistic:.2f}$'
                )
            ...
            try:
                annotation_text = (
                    f'$\\gamma = P(X \\geq x) = {evaluation_result.quantile:.2f}$\n'
                    f'$\\omega = {evaluation_result.observed_statistic:.2f}$'
                )
            except TypeError:
                annotation_text = (
                    f'$\\gamma = P(X \\geq x) = {evaluation_result.quantile[0]:.2f}$\n'
                    f'$\\omega = {evaluation_result.observed_statistic:.2f}$'
                )
            ...
            raise ValueError(f"Unsupported evaluation_result type: {type(evaluation_result)}")
    ...
    return xlabel, ylabel, annotation_text, annotation_xy